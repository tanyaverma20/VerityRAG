...
def test_real_pdf_upload_still_works():
    ...
    with open(FIXTURE_PDF, "rb") as f:
        resp = client.post("/upload", files={"file": ("attention.pdf", f, "application/pdf")})
...
def test_upload_rejects_a_file_that_is_not_really_a_pdf():
    ...
    resp = client.post("/upload", files={"file": ("fake.pdf", fake_pdf, "application/pdf")})
...
def test_upload_rejects_a_file_over_the_size_limit(monkeypatch):
    ...
    resp = client.post("/upload", files={"file": ("big.pdf", oversized, "application/pdf")})
